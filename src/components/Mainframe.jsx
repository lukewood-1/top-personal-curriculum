...
import PersonalInfoContainer from './PersonalInfo.jsx'
import Objective from './Objective.jsx'
import AcademicContainer, { AcademicEntry } from './AcademicSection.jsx'
import ExperienceContainer, { ExperienceEntry } from './ExperienceContainer.jsx'
import AdditionalInfoContainer, { AdditionalInfoEntry } from './AdditionalInfoContainer.jsx'
import SkillsContainer, { SkillEntry } from './SkillsContainer.jsx';

export default function Mainframe(){
  const [isPrint, setIsPrint] = useState(true);
  const [canLegend, setCanLegend] = useState(true);

  const handleToggleLegend = () => setCanLegend(!canLegend);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('afterprint', offPrint);
    }
  }, [isPrint]);
...
// Add floating window with help legends
  function LegendModal(){
    const [info, setInfo] = useState(
      {transform: 'scaleX(-1)', x: '0px', y: '0px', title: '', description: '', show: '0'}
    );
